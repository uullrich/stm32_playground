...
CanBus::CanBus(CAN_HandleTypeDef& hcan) noexcept : hcan_{&hcan} {}
...
{
    while (!tx_queue_.is_empty() && HAL_CAN_GetTxMailboxesFreeLevel(hcan_) > 0)
    {
        CanMessage msg;
        if (!tx_queue_.pop(msg))
            break;
